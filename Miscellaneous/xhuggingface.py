...
class XHFLM(TemplateLM):
    ...
    def _create_model(
        ...
        **kwargs,
        ...
        if attentionimplementation == "sdpa": 
            print("SDPA") 
            self._model.config._attn_implementation = "sdpa" 
        else: 
            print("General") 
            self._model.config._attn_implementation = "general" 
        ...
        self._model.config.secondrollback = secondrollback 
        self._model.config.treewidth = widthtree 
        self._model.config.filteractiveenabled = filteractiveenabled 
        self._model.config.context_length = contextlength # context length 
        #self._model.eval()
        if griffin:
            self._model = get_llama_griffin2(self._model, schedule_k)
    ...
    def generate_until(
        ...
        headers = [] 
        data = [] 
        print("idx {} start communication".format(self.accelerator.process_index)) 
        dist.barrier() 
        num_sentence = self._model.num_sentence 
        totalgenerationlength = self._model.totalgenerationlength 
        ...
        if self._model.config.check:
            total_step = self._model.total_steps 
            num_step = self._model.num_steps 
