...
def get_llama_griffin2two(model,  k_schedule):
    config = model.config
...
def get_llama_griffin2(model,  k_schedule):
    config = model.config
    for i, l in enumerate(model.model.layers):
        new_mlp = GriffinLlamaGathergemv(config, k_schedule[i]) 
        new_mlp.gate_proj = l.mlp.gate_proj
        new_mlp.up_proj = l.mlp.up_proj
        new_mlp.down_proj = l.mlp.down_proj
        new_mlp.act_fn = l.mlp.act_fn
...
class GriffinLlamaGathergemv(nn.Module): 
    def __init__(self, config, k_factor):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
    ...
    def forward(self, x): 
        ...
        if self.inference_mode == "full": 
            ...
            k = int(int_states.shape[-1] * k_factor) 
            states = int_states[:, self.reference:, :] 
            neuron_stat = ((states/states.norm(dim = -1, keepdim = True))) # batch_size, hidden_size 
            neuron_stat = neuron_stat.norm(dim = 1) 
            if self.neuron_stat is None: 
                self.neuron_stat = neuron_stat 
                stat = self.neuron_stat 
            ...
            # here let's first try using the sparse matrix multiplication and compare whether it is faster than using smaller dense matrix multiplcation 
            from ops.triton.gather_gemv import mlp_sparse2 
            
            assert x.shape[1] == 1 # make sure sequence length is 1 
            down_proj = mlp_sparse2(
                x = x.squeeze(1), 
                W1 = self.gate_proj.weight, 
                W3 = self.up_proj.weight, 
                W2t = self.down_proj.weight.t(), 
                idx = self.topk_indices[0], 
                bias1 = None, 
                bias2 = None, 
            ) 
            down_proj = down_proj.unsqueeze(1) 
