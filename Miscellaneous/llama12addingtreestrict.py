...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    @staticmethod 
    def contains_contiguous_subsequence2(tensor, vector):
        b, N = tensor.shape
        M = vector.shape[0]

        # Create a sliding window of shape (b, N - M + 1, M)
        sliding_windows = tensor.unfold(1, M, 1)

        # Check if any of the windows match the vector
        match = (sliding_windows == vector).all(dim=-1)

        # If any match is found, return True, else return False 
        # match = match.view(-1) 
        match_batch_wise = match.any(dim = -1) # match_batch_wise has shape (b, N - M + 1) 
        batch_idx = torch.where(match_batch_wise)[0] 
        if batch_idx.numel() > 0: 
            return batch_idx[0].item() 
        else: 
            return -1 
        # return match.any(dim=-1) 
    
    @staticmethod 
    def contains_contiguous_subsequence(tensor, vector): 
        b, N = tensor.shape
        M = vector.shape[0] 
        sliding_windows = tensor[:, : vector.shape[0]] 
        match = (sliding_windows == vector).all(dim = -1) 
        batch_idx = torch.where(match)[0] 
        if batch_idx.numel() > 0: 
            return batch_idx[0].item() 
        else: 
            return -1 
        ...
        while True: 
            ...
            if self.config.check: 
                if (currentlength - last_check) == kernel_size or want_to_quit: 
                    # collecting drafting tree size 
                    self.averagedraftingbatchsize += self.batchsizecount/kernel_size 
                    # print("draft batchsize count {} averagedraftingbatchsize {}".format(self.batchsizecount/kernel_size, self.averagedraftingbatchsize)) 
                    self.batchsizecount = 0 
                    check_flag = True 
                    past_key_values = model_inputs["past_key_values"] 
                    # past_key_values = outputs.past_key_values 
                    ...
                    # print("flattentreesize {}".format(self.flattentreesize)) 
                    ...
                    old_select_idx = 0 
                    for k in range(selected_likelihood.shape[1]): 
                        if selected_likelihood[select_idx][k] >= self.config.thr: 
                            lengthaccepts += 1 
                        else: 
                            next_token = torch.argmax(full_predicted_logits[select_idx][lengthaccepts], dim = -1).unsqueeze(dim = 0) 
                            targetsequence = torch.cat([sparse_predicted_tokens[select_idx][: lengthaccepts], next_token], dim = -1) 
                            old_select_idx = select_idx 
                            select_idx = LlamaForCausalLM.contains_contiguous_subsequence(sparse_predicted_tokens, targetsequence) 
                            if select_idx == -1: 
                                acceptsall = False 
                                break 
                            else: 
                                lengthaccepts += 1 
                                # print(colored("finding sequence {} index chosen to be {}".format(self.tokenizer.decode(targetsequence), select_idx), "yellow"), flush = True) 
                                # print("sequence now * {} * indedx at {}".format(self.tokenizer.decode(sparse_predicted_tokens[old_select_idx][: lengthaccepts]), old_select_idx)) 
                                # print("find sequence * {} * index at {}".format(self.tokenizer.decode(targetsequence), select_idx)) 
                                # time.sleep(10) 
                    ...
                    if lengthaccepts > lengthacceptsbig: 
                        lengthacceptsbig = lengthaccepts 
                        batchdimselection = select_idx if select_idx != -1 else old_select_idx 
                    requireallpasses = requireallpasses or acceptsall 
                    ...
                    # print("total_steps {} num_steps {} lengthacceptsbig {}".format(self.total_steps, self.num_steps, lengthacceptsbig)) 
                    # print("number of total steps now is {}".format(self.total_steps / self.num_steps)) 
                    ...
                    # self.total_steps += lengthacceptsbig + 1 
                    if lengthacceptsbig != checklength - 1: 
                        keylooking = torch.cat([sparse_predicted_tokens[batchdimselection][: lengthacceptsbig], next_tokens], dim = 0) 
                        findingvariab = LlamaForCausalLM.contains_contiguous_subsequence(sparse_predicted_tokens[1 :], keylooking) != -1 
                        # if not findingvariab: 
                            # print(colored("We didn't find the key look", "red")) 
                        if findingvariab: 
                            # print(colored("We found the key look", "green")) 
                            self.findinggreedydecoded += 1 
                            # print("number accumulated now is {}".format(self.findinggreedydecoded)) 
