...
def get_llama_griffin2(model,  k_schedule):
    ...
    for i, l in enumerate(model.model.layers):
        ...
        new_mlp = LlamaMLPnotnorma(config, 0.0).to(model.device).to(torch.bfloat16) 
        # new_mlp.gate_proj = l.mlp.gate_proj 
        # new_mlp.up_proj = l.mlp.up_proj 
        # new_mlp.down_proj = l.mlp.down_proj 
        # new_mlp.act_fn = l.mlp.act_fn 
...
def get_llama_normal(model): 
    config = model.config
    for i, l in enumerate(model.model.layers):
        new_mlp = LlamaMLPnormal(config, 0.0)
        new_mlp.gate_proj = l.mlp.gate_proj
        new_mlp.up_proj = l.mlp.up_proj
        new_mlp.down_proj = l.mlp.down_proj
        new_mlp.act_fn = l.mlp.act_fn
...
class LlamaMLPnormal(nn.Module): 
    def __init__(self, config, k_factor):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
...
class GriffinLlamaMLP2(nn.Module): # Griffin-like 
    ...
    def forward(self, x):
            k_factor = self.k_factor 
            # if self.tokindices == None: 
            #     tok_indices = torch.arange(0, int(self.intermediate_size * k_factor), device = x.device) 
            #     tok_indices = tok_indices.unsqueeze(0).expand(x.shape[0], -1) # B, k 
            #     self.prepare_reduced_weights(tok_indices) 
            #     self.tokindices = tok_indices 
            #     print("using shape {}".format(tok_indices.shape)) 
            if self.mode == 'gen':
                if self.inference_mode == 'full':
                    ...
                    # torch.cuda.synchronize() 
                    print("Time single forward pass {}".format(endtime - starttime)) 
                    # print(colored("Time taken: {}".format(self.start_event.elapsed_time(self.end_event)), "blue")) 
                    exit(0) 
                    ''' 
                else:
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        allowprofiling: Optional[bool] = False,
        **kwargs,
        ...
        # old_hidden_states = hidden_states.clone() # storing the hidden states value 
        # if allowprofiling: 
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        causal_mask = None # remove this line 
        # embed positions
    ...
    def _update_causal_mask(self, attention_mask, input_tensor, cache_position):
        ...
        # print(colored("Causal mask shape {}".format(causal_mask.shape), "blue")) 
        # print(target_length) 
        # print("cache_position shape {}".format(cache_position.shape)) 
        ...
        while input_ids.shape[1] - initial_len < 50: 
            ...
            if generatedlength == 0: 
                ...
                for i in range(100): 
                    ...
                    # past_key_values.rollback(steps = 1) 
                print("warming up") 
